...
export default function SubjectTopicDetailPage({
  ...
  // 1. Fetch Subject group info
  const { data: subject } = useQuery<SubjectGroup>({
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-2 pb-4 border-b border-border/30">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-xs font-bold text-rose-500 font-mono uppercase tracking-wider">
            ...
            <span>Subject Topic Note</span>
            <span>·</span>
            <span>{subject?.name || subjectSlug}</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>Back to {subject?.name || 'Subject Topics'}</span>
          </Link>
          {isAdmin && (
            <Link
